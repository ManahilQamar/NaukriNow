...
const PostJob = () => {
  const [formData, setFormData] = useState({
    ...
    jobType: "Full-time",
    jobLocationType: "",
    payType: "range", // range or starting amount
    minSalary: "",
    maxSalary: "",
    payRate: "per month", // per hour, per day, per week, per month, per year
  });
  ...
  const calculateEstimatedPay = () => {
    ...
    if (payType === "range" && minSalary && maxSalary) {
      ...
      if (payRate === "per hour") {
        min = min * 160; // approximate monthly (40 hrs/week * 4 weeks)
        max = max * 160;
      } else if (payRate === "per day") {
        min = min * 20; // approximate monthly (20 working days)
        max = max * 20;
      } else if (payRate === "per week") {
        min = min * 4; // approximate monthly
        max = max * 4;
      } else if (payRate === "per year") {
        min = min / 12; // monthly equivalent
        max = max / 12;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    // Format salary based on pay type and rate
    let formattedSalary = "";
